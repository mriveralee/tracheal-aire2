...
// TODO Use this in the DWV functionality to get an input box associated with this node for changing values based onku
  ...
  createParamControls: function() {
    ...
    for(var i = 0; i < this.paramDefinitions.length; i++)
    {
      ...
      if( (type == "text") || (type == "int") || (type == "float") )
      {
        ...
        control.type = "text";
        // TODO: Use these ids to pick selected Parameter
        control.id = "input-param-" + i;
      ...
      // Add listener to get selected parameter
      // Fxn used for handling the selected input parameters
      var FOCUSED_PARAM_CLASS = 'focused-parameter';
      var handleSelectedInputParam = function(e, selectedId) {
        if (TA_SELECTED_PARAMETER_ID && TA_SELECTED_PARAMETER_ID.length > 0) {
          // Remove old focus
          var oldDiv = document.getElementById(TA_SELECTED_PARAMETER_ID);
          oldDiv.className = oldDiv.className.replace(FOCUSED_PARAM_CLASS,'');
          // Associated Old Text
          var oldText = oldDiv.parentElement.parentElement.children[0];
          oldText.className = oldText.className.replace('btn-focused','');
          //$("label[for='"+$(oldDiv).attr('id')+"']").;
        }
        // Now set selected focus
        if (selectedId.length > 0) {
          TA_SELECTED_PARAMETER_ID = selectedId;
          // Add focus of box / css
          var paramDiv = document.getElementById(TA_SELECTED_PARAMETER_ID);
          paramDiv.className = paramDiv.className + ' ' + FOCUSED_PARAM_CLASS;
          var newText = paramDiv.parentElement.parentElement.children[0]
          newText.className = newText.className + ' ' + 'btn-focused';
          control.addEventListener("click", function(e) {
            handleSelectedInputParam(e, this.id)
          });


        } else {
          // Remove Focus and reset selection
          TA_SELECTED_PARAMETER_ID = null;
        }
        console.log("The selected id is", selectedId, "the TA is: ", TA_SELECTED_PARAMETER_ID );
      };

      control.addEventListener("click", function(e) { handleSelectedInputParam(e, this.id)});
      ...
      td.innerHTML = (control.id.length > 0)
        ? ("<label for='" + control.id + "'>" + label + "</label>") : label;
      tr.appendChild(td);
...
function onload() {
   gProcessor = new OpenJsCad.Processor(document.getElementById("tracheal-aire-viewer"));
}
